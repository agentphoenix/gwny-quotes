<div class="row">
	<div class="col-md-6">
		<div class="panel panel-default bootcards-media">
			<div class="panel-heading">
				<h3 class="panel-title">Rochester, NY</h3>
			</div>
			{{ HTML::image('img/rochester.jpg', null, ['class' => 'img-responsive']) }}
			<div class="panel-footer">
				<div class="row">
					<div class="col-sm-6">
						<div class="visible-xs">
							<p><a href="http://golfwny.com/rochester/courses" target="_blank" class="btn btn-default btn-lg btn-block">More Info</a></p>
						</div>
						<div class="hidden-xs">
							<a href="http://golfwny.com/rochester/courses" target="_blank" class="btn btn-default btn-block">More Info</a>
						</div>
					</div>
					<div class="col-sm-6">
						<div class="visible-xs">
							<p><a href="{{ route('home', ['rochester', 'info']) }}" class="btn btn-primary btn-lg btn-block">Get Started</a></p>
						</div>
						<div class="hidden-xs">
							<a href="{{ route('home', ['rochester', 'info']) }}" class="btn btn-primary btn-block">Get Started</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>

	<div class="col-md-6">
		<div class="panel panel-default bootcards-media">
			<div class="panel-heading">
				<h3 class="panel-title">Buffalo, NY</h3>
			</div>
			{{ HTML::image('img/buffalo.jpg', null, ['class' => 'img-responsive']) }}
			<div class="panel-footer">
				<div class="row">
					<div class="col-sm-6">
						<div class="visible-xs">
							<p><a href="http://golfwny.com/buffalo/courses" target="_blank" class="btn btn-default btn-lg btn-block">More Info</a></p>
						</div>
						<div class="hidden-xs">
							<a href="http://golfwny.com/buffalo/courses" target="_blank" class="btn btn-default btn-block">More Info</a>
						</div>
					</div>
					<div class="col-sm-6">
						<div class="visible-xs">
							<p><a href="{{ route('home', ['buffalo', 'info']) }}" class="btn btn-primary btn-lg btn-block">Get Started</a></p>
						</div>
						<div class="hidden-xs">
							<a href="{{ route('home', ['buffalo', 'info']) }}" class="btn btn-primary btn-block">Get Started</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>

	<div class="col-md-6">
		<div class="panel panel-default bootcards-media">
			<div class="panel-heading">
				<h3 class="panel-title">Silver Lake Resort</h3>
			</div>
			{{ HTML::image('img/silverlake.jpg', null, ['class' => 'img-responsive']) }}
			<div class="panel-footer">
				<div class="row">
					<div class="col-sm-6">
						<div class="visible-xs">
							<p><a href="http://golfwny.com/rochester/course/silver-lake-country-club" target="_blank" class="btn btn-default btn-lg btn-block">More Info</a></p>
						</div>
						<div class="hidden-xs">
							<a href="http://golfwny.com/rochester/course/silver-lake-country-club" target="_blank" class="btn btn-default btn-block">More Info</a>
						</div>
					</div>
					<div class="col-sm-6">
						<div class="visible-xs">
							<p><a href="http://golfwny.com/rochester/book-package" target="_blank" class="btn btn-primary btn-lg btn-block">Get Started</a></p>
						</div>
						<div class="hidden-xs">
							<a href="http://golfwny.com/rochester/book-package" target="_blank" class="btn btn-primary btn-block">Get Started</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>

	<!--<div class="col-md-6">
		<div class="panel panel-default bootcards-media">
			<div class="panel-heading">
				<h3 class="panel-title">Reservoir Creek</h3>
			</div>
			{{ HTML::image('img/reservoircreek.jpg', null, ['class' => 'img-responsive']) }}
			<div class="panel-footer">
				<div class="row">
					<div class="col-sm-6">
						<div class="visible-xs">
							<p><a href="http://golfwny.com/rochester/course/reservoir-creek-golf-course--inn" target="_blank" class="btn btn-default btn-lg btn-block">More Info</a></p>
						</div>
						<div class="hidden-xs">
							<a href="http://golfwny.com/rochester/course/reservoir-creek-golf-course--inn" target="_blank" class="btn btn-default btn-block">More Info</a>
						</div>
					</div>
					<div class="col-sm-6">
						<div class="visible-xs">
							<p><a href="http://golfwny.com/rochester/book-package" target="_blank" class="btn btn-primary btn-lg btn-block">Get Started</a></p>
						</div>
						<div class="hidden-xs">
							<a href="http://golfwny.com/rochester/book-package" target="_blank" class="btn btn-primary btn-block">Get Started</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>-->

	<div class="col-md-6">
		<div class="panel panel-default bootcards-media">
			<div class="panel-heading">
				<h3 class="panel-title">Ellicott Villas</h3>
			</div>
			{{ HTML::image('img/ellicott-villas.jpg', null, ['class' => 'img-responsive']) }}
			<div class="panel-footer">
				<div class="row">
					<div class="col-sm-6">
						<div class="visible-xs">
							<p><a href="http://golfwny.com/buffalo/course/ellicott-villa-golf-packages" target="_blank" class="btn btn-default btn-lg btn-block">More Info</a></p>
						</div>
						<div class="hidden-xs">
							<a href="http://golfwny.com/buffalo/course/ellicott-villa-golf-packages" target="_blank" class="btn btn-default btn-block">More Info</a>
						</div>
					</div>
					<div class="col-sm-6">
						<div class="visible-xs">
							<p><a href="http://golfwny.com/buffalo/book-package" target="_blank" class="btn btn-primary btn-lg btn-block">Get Started</a></p>
						</div>
						<div class="hidden-xs">
							<a href="http://golfwny.com/buffalo/book-package" target="_blank" class="btn btn-primary btn-block">Get Started</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>